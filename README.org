...
** Widget selection (legacy)

When using the widget registry (recommended), this variable is ignored. See [[*Widget Registration][Widget Registration]] below.

#+begin_src emacs-lisp
;; Legacy: manual widget list (used only if registry is empty)
